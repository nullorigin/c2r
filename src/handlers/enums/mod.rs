...
impl Build for EnumDef {
    fn to_entry(&self) -> Entry {
        ...
        let variant_entries: Vec<Entry> = self.variants.iter().map(|v| {
...
impl Processor for EnumHandler {
    ...
    fn supported_patterns(&self) -> &[&str] {
        &["enum_definition", "typedef_enum"]
    }
    
    fn patterns(&self) -> Vec<Pattern> {
        vec![
            Pattern::definition(500, "enum_definition", vec![
            ...
            Pattern::definition(501, "typedef_enum", vec![
    ...
    fn convert(&mut self) -> Option<String> {
        ...
        crate::system::system_mut().register_enum(
        ...
        for variant in &self.data.main_enum.variants {
            if let Some(value) = variant.value {
                ...
                output.push_str(&format!("    {} = {}, // TODO: evaluate expression\n", variant.name, expr));
            } else {
                output.push_str(&format!("    {},\n", variant.name));
            }
        }
        ...
        if let Some(ref alias) = self.data.typedef_alias {
            if alias != &self.data.main_enum.name {
                output.push_str(&format!("\npub type {} = {};", alias, self.data.main_enum.name));
            }
        }
...
impl Build for EnumHandler {
    fn to_entry(&self) -> Entry {
        ...
        entry.set_attr("variant_count", Entry::i64(self.data.main_enum.variants.len() as i64));
    ...
    fn name(&self) -> Option<&str> { Some("EnumHandler") }
    fn category(&self) -> Option<&str> { Some("enum") }
}
