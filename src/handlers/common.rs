...
pub fn is_type(tokens: &[Token], context: &Context) -> bool {
    if tokens.is_empty() {
        return false;
    }
    ...
    if is_token(first_token, "const") && tokens.len() > 1 {
        return is_type(&tokens[1..], context);
    }

    // Check for typedefs in the registry
    if let Some(typedefs) = context.registry.entries.get(&Id::new(0,"typedefs".to_string())) {
        match typedefs {
            crate::context::Entry::StrMap(map) => {
...
pub fn not_handled() -> Result<HandlerResult, ConversionError> {
    Ok(HandlerResult::NotHandled(None, get_id("not_handled")))
}
...
pub fn replace_with(rust_code: String,id:Id) -> Result<HandlerResult, ConversionError> {
    Ok(HandlerResult::Replaced(None, rust_code, id))
}
