...
impl Processor for CallHandler {
    ...
    fn patterns(&self) -> Vec<Pattern> {
        vec![
            Pattern::definition(500, "function_call", vec![
    ...
    fn extract(&mut self, tokens: &[Token]) -> bool {
        ...
        // Check for method call (obj.method or obj->method)
        if paren_pos >= 2 {
    ...
    fn convert(&mut self) -> Option<String> {
        ...
        let rust_code = if self.data.is_method_call {
            ...
            let func_name = self.id_converter.convert(&self.data.name)
...
impl CallHandler {
    ...
    fn convert_argument(&mut self, tokens: &[String]) -> String {
        ...
        let converted: Vec<String> = tokens.iter()
            .map(|t| {
