...
impl Processor for CommentHandler {
    ...
    fn patterns(&self) -> Vec<Pattern> {
        vec![
            Pattern::definition(0, "line_comment", vec![
                    PatternRule::custom("starts_with_line_comment"),
                ])
            ...
            Pattern::definition(1, "block_comment", vec![
                    PatternRule::custom("starts_with_block_comment"),
                ])
                .with_category("comment")
                .with_priority(100)
                .with_min_tokens(1)
    ...
    fn validate(&mut self, tokens: &[Token]) -> bool {
        ...
        if first.starts_with("//") {
            ...
            self.confidence = if best_confidence > 0.0 { best_confidence } else { 1.0 };
            true
    ...
    fn extract(&mut self, tokens: &[Token]) -> bool {
        ...
        // For comments, the entire token is the comment
        ...
        match self.data.kind {
            CommentKind::Line | CommentKind::Doc => {
                ...
                self.data.text = full_comment.chars().skip(prefix_len).collect::<String>().trim().to_string();
            }
            CommentKind::Block => {
                // Remove /* and */ or /** and */
...
impl Build for CommentHandler {
    fn to_entry(&self) -> Entry {
        ...
        attrs.insert("kind".to_string(), Entry::string(match self.data.kind {
...
mod tests {
    ...
    fn test_doc_comment() {
        ...
        assert_eq!(handler.output(), Some("/// Documentation comment".to_string()));
    }
