...
pub const MAX_PRECISION: u64 = 576460752303423500;
...
// `e` notation.
pub fn allow_number_extensions(
    parser: &mut Parser,
    mut num: u64,
    mut e: i16,
    ch: u8,
...
pub fn allow_number_extensions_zero(parser: &mut Parser) -> Result<Number> {
    if parser.is_eof() {
        Ok(0.into())
    } else {
        let num = 0;
...
pub fn expect_fraction(parser: &mut Parser, num: &mut u64, e: &mut i16) -> Result<Number> {
