...
use crate::{AdaptivePatternResult, C2RError, Context, Entry, HandlerPattern, HandlerPhase, HandlerReport, HandlerResult, Handlizer, Id, Justification, Kind, Patternizer, Reason, ReportLevel, Result, SegmentType, Table, TableCell, Token, context, handler, handlers::{
        array_handler::ArrayHandler, comment_handler::CommentHandler, enum_handler::EnumHandler, expression_handler::ExpressionHandler, function_handler::FunctionHandler, global_handler::GlobalHandler, macro_handler::MacroHandler, struct_handler::StructHandler, typedef_handler::TypedefHandler
    }, info, pattern::Pattern, time
};
use crate::Handler;
use std::{any::Any, path::PathBuf, sync::Arc};
use core::{option::Option::{self, None, Some}, result::Result::Ok};
use std::ops::Range;
...
pub struct Registry {
    pub id: Id,
    pub reports: HashMap<Id, HandlerReport>, // Direct report storage for fast access
...
impl Clone for Registry {
    fn clone(&self) -> Self {
        Self {
            id: self.id.clone(),
            reports: self.reports.iter().map(|(id, report)| (id.clone(), report.clone())).collect(),
            entries: self.entries.iter().map(|(id, entry)| (id.clone(), entry.clone())).collect(),
...
impl Registry {
    ...
    pub fn get_handlers_for_pattern(pattern: &str) -> Vec<Id> {
        // Temporarily return empty to avoid deadlock
        // TODO: Fix by passing handlizer as parameter instead of using global context
        Vec::new()
    }
...
pub fn add_report(&mut self, report: HandlerReport) {
    ...
    let report_id = Id::get(&format!("report_{}_{}", report.handler_name, self.reports.len()));
...
pub fn create_report(&mut self, handler_id: Id, handler_name: String, message: String, 
                    level: ReportLevel, phase: HandlerPhase, success: bool, 
                    tokens_processed: u64, tokens_consumed: u64) -> Id {
    let timestamp = time();
    let report_id = Id::get(&format!("report_{}", timestamp));
    
    let report = HandlerReport {
        report_id: std::sync::Arc::new(report_id.clone()),
        handler_id: std::sync::Arc::new(handler_id),
        handler_name,
        function_name: "process".to_string(),
        message,
        level,
        tokens_processed: tokens_processed as usize,
        tokens_consumed: tokens_consumed as usize,
        phase,
        success,
        details: None,
...
pub fn get_successful_reports(&self) -> Vec<HandlerReport> {
    self.reports.values()
...
pub fn get_failed_reports(&self) -> Vec<HandlerReport> {
    self.reports.values()
        .filter(|report| !report.success)
        .cloned()
        .collect()
...
pub fn clear_reports(&mut self) {
    ...
    let report_ids: Vec<Id> = self.entries
        .iter()
        .filter_map(|(id, entry)| {
            match entry {
...
pub fn display_formatted_reports(&self, format: &ReportFormat) {
    ...
    let reports: Vec<&HandlerReport> = self.reports.values().collect();
    ...
    match format {
        ReportFormat::Text => {
            ...
            println!("📊 Summary: {} total | ✅ {} success | ❌ {} errors | ⚠️ {} warnings", 
                total_reports, successful_reports, error_reports, warning_reports);
            
            // Group reports by handler for better organization
            let mut handler_groups: HashMap<String, Vec<&HandlerReport>> = HashMap::new();
            for report in &reports {
                handler_groups.entry(report.handler_name.clone())
        ...
        ReportFormat::Table => {
            ...
            println!("📊 {} total reports | Success rate: {:.1}%", 
                total_reports, 
                if *total_reports > 0 { (*successful_reports as f64 / *total_reports as f64) * 100.0 } else { 0.0 }
            );
        ...
        ReportFormat::Compact => {
            ...
            println!("📊 {} total | ✅ {} OK | ❌ {} ERR | ⚠️ {} WARN | 🔵 {} INFO", 
                total_reports, successful_reports, error_reports, warning_reports,
            ...
            for report in sorted_reports {
                ...
                println!("{} {} [{}] {}: {}", 
                    status_icon, level_icon, report.handler_name, 
...
fn format_report_summary(&self, report: &HandlerReport) -> String {
    ...
    format!("{} {} [{:?}] {}", 
        status, level_icon, report.phase, 
...
pub fn display_formatted_reports_by_level(&self, level: &ReportLevel, format: &ReportFormat) {
    ...
    match format {
        ...
        ReportFormat::Compact => {
            println!("\n=== {:?} LEVEL SUMMARY ({} reports) ===", level, report_count);
            for report in &reports {
                println!("[{}] {}: {}", 
...
pub fn display_report(&self, report: &HandlerReport) {
    let status_icon = if report.success { "✓" } else { "✗" };
    ...
    pub fn generate_report(&self, context: &Context) -> Result<String> {
        ...
        let handler_data: Vec<_> = context.handlizer.handlers.iter()
            .map(|(id, handler)| {
                let handler_name = id.name().to_string();
                let handler_reports:Vec<HandlerReport> = self.get_reports_by_handler(&handler_name);
                let success_count = self.get_successful_reports().len();
                let success_rate = if handler_reports.is_empty() { 0.0 } else {
                    (success_count as f64 / handler_reports.len() as f64) * 100.0
                };
                
                (handler_name, handler.role(), handler.supported_patterns().to_vec(), 
                 handler_reports.len(), success_rate)
            })
            .collect();
    ...
    pub fn get_report_stats(&self) -> HashMap<String, u64> {
        ...
        stats.insert("successful_reports".to_string(), self.get_successful_reports().len() as u64);
        stats.insert("failed_reports".to_string(), self.get_failed_reports().len() as u64);
        stats.insert("error_reports".to_string(), self.get_reports_by_level(&ReportLevel::Error).len() as u64);
        stats.insert("warning_reports".to_string(), self.get_reports_by_level(&ReportLevel::Warning).len() as u64);
        stats.insert("info_reports".to_string(), self.get_reports_by_level(&ReportLevel::Info).len() as u64);
        stats.insert("debug_reports".to_string(), self.get_reports_by_level(&ReportLevel::Debug).len() as u64);
        stats
    }
    
    /// Get all handler reports from the Entry system
    pub fn get_handler_reports_from_entries(&self) -> Vec<HandlerReport> {
        self.entries
            .values()
    ...
    pub fn add_report_entry(&mut self, name: &str, report: HandlerReport) -> Id {
        let id = Id::get(name);
    ...
    fn create_reports_table(&self, reports: &[HandlerReport]) -> Table {
        ...
        // Create header row
        let headers = ["Status", "Handler", "Level", "Phase", "Tokens P/C", "Message"];
        ...
        for (row, report) in reports.iter().enumerate() {
            ...
            // Status column
            let status = if report.success { "✓" } else { "✗" };
            let mut status_cell = TableCell::new_with_justification(data_row, 0, &format!("status_{}", row), Justification::Center);
            ...
            // Handler column
            let mut handler_cell = TableCell::new_with_justification(data_row, 1, &format!("handler_{}", row), Justification::Left);
            if let Some(entry) = self.create_table_entry(report.handler_name.clone()) {
                handler_cell.add_entry(entry);
            }
            table.set_cell(data_row, 1, handler_cell);
            
            // Level column
            let mut level_cell = TableCell::new_with_justification(data_row, 2, &format!("level_{}", row), Justification::Center);
            ...
            // Phase column
            let mut phase_cell = TableCell::new_with_justification(data_row, 3, &format!("phase_{}", row), Justification::Center);
            ...
            let mut tokens_cell = TableCell::new_with_justification(data_row, 4, &format!("tokens_{}", row), Justification::Right);
            ...
            let mut message_cell = TableCell::new_with_justification(data_row, 5, &format!("message_{}", row), Justification::Left);
    ...
    pub fn create_stats_table(&self) -> Table {
        ...
        // Header row
        let mut stat_name_header = TableCell::new_with_justification(0, 0, "stat_name_header", Justification::Center);
        if let Some(entry) = self.create_table_entry("Statistic".to_string()) {
            stat_name_header.add_entry(entry);
        }
        let mut stat_value_header = TableCell::new_with_justification(0, 1, "stat_value_header", Justification::Center);
        ...
        // Add stats data
        let stat_order = ["total_reports", "successful_reports", "failed_reports", "error_reports", "warning_reports", "info_reports", "debug_reports"];
        let stat_labels = ["Total Reports", "Successful", "Failed", "Errors", "Warnings", "Info", "Debug"];
        ...
        for (row, (stat_key, label)) in stat_order.iter().zip(stat_labels.iter()).enumerate() {
            ...
            // Stat name cell
            let mut name_cell = TableCell::new_with_justification(data_row, 0, &format!("stat_name_{}", row), Justification::Left);
            ...
            // Stat value cell
            let mut value_cell = TableCell::new_with_justification(data_row, 1, &format!("stat_value_{}", row), Justification::Right);
    ...
    pub fn save_pattern_cache_to_file(&self,cache_dir: &str) -> Result<()> {
        use std::fs;
        use std::path::Path;
        ...
        // TODO: Fix deadlock - avoid global context access during processing
        // For now, skip cache save to avoid deadlock
        // Note: Pattern cache save skipped to avoid deadlock
    ...
    pub fn load_pattern_cache_from_file(&mut self,context: &mut Context, cache_dir: &str) -> Result<()> {
        use std::path::Path;
        ...
        if cache_file.exists() {
            // Get mutable context to load cache
            let _ = context.load_pattern_cache(&cache_path);
    ...
    fn get_cached_tokens_for_range(&self, _active_slot: usize, token_range: Range<usize>) -> Vec<Token> {
    ...
    fn from_value<T: Any + Clone + std::fmt::Debug>(&self, value: T) -> Entry {
        ...
        if let Some(v) = any_value.downcast_ref::<Id>() {
            ...
            Entry::Path(v.clone())
        } else if let Some(v) = any_value.downcast_ref::<HandlerReport>() {
            ...
            Entry::StrMap(v.iter().map(|(k, v)| (k.clone(), Entry::Str(v.clone()))).collect())
        } else if let Some(v) = any_value.downcast_ref::<Pattern>() {
            Entry::Patternizer(v.clone())
        } else {
            panic!("Unsupported type for Entry conversion: {:?}", any_value.type_id())
        }
    }
    pub fn insert_pattern(&mut self, pattern_name: &str, pattern: Pattern) {
        let key = format!("pattern_{}", pattern_name);
        let id = Id::get(&key);
        self.insert(id, Entry::Patternizer(pattern));
    }
    ...
    pub fn get_pattern(&self, pattern_name: &str) -> Option<Pattern> {
        ...
        match self.entries.get(&id) {
            ...
            Some(_) => {
                info!("⚠️ Entry '{}' exists but is not a Pattern type", pattern_name);
                None
            }
            None => {
    ...
    pub fn remove_patterns(&mut self, pattern: &str) -> Vec<Entry> {
        let ids_to_remove: Vec<Id> = self.entries
            .keys()
