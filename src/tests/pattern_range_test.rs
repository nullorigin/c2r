...
mod range_tests {
    ...
    fn test_range_offsets_serialization() {
        ...
        if let Some(PatternResult::CountOf { offsets }) = new_patternizer.match_cache.get("count_ranges") {
        ...
        if let Some(PatternResult::Fuzzy { offsets }) = new_patternizer.match_cache.get("fuzzy_ranges") {
            assert_eq!(offsets.len(), 3);
