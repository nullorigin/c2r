#[cfg(test)]
mod tests {
    use crate::pattern::{Patternizer, PatternResult};
    use std::fs;
    ...
    fn test_pattern_cache_save_load() {
        ...
        patternizer.match_cache.insert(
            "count_key".to_string(),
            PatternResult::CountOf { offsets: vec![1..2, 3..4, 7..8] }
        );
        ...
        match result {
            Ok(()) => {
                ...
                if let Ok(content) = fs::read_to_string(temp_file) {
                    ...
                    println!("Cache content preview: {}", &content[..content.len().min(200)]);
    ...
    fn test_cache_with_all_pattern_result_types() {
        ...
        patternizer.match_cache.insert("match".to_string(), 
            PatternResult::Match { consumed_tokens: 10 });
        patternizer.match_cache.insert("nomatch".to_string(), 
            PatternResult::NoMatch { reason: "No pattern found".to_string() });
        patternizer.match_cache.insert("countof".to_string(), 
            PatternResult::CountOf { offsets: vec![0..1, 2..3, 4..5, 6..7] });
        patternizer.match_cache.insert("sequence".to_string(), 
            PatternResult::Sequence { range: 5..15 });
        patternizer.match_cache.insert("fuzzy".to_string(), 
            PatternResult::Fuzzy { offsets: vec![1..2, 5..6, 9..10] });
        patternizer.match_cache.insert("reject".to_string(), 
            PatternResult::Reject { reason: "Pattern rejected".to_string() });
