...
fn main() {
    ...
    match command.as_str() {
        ...
        "convert" | "" if !input_file.is_empty() => {
            if let Err(e) = convert(&input_file, &output_file, output_to_stdout) {
...
fn convert(
    ...
    use crate::db::Tokenizer;
    use crate::system::ProcessMode;
    ...
    // Tokenize the C source
    let mut tokenizer = Tokenizer::new();
    tokenizer.set_content(content.as_bytes().to_vec());
    let slot_id = tokenizer.tokenize();
    let tokens = tokenizer.get_slot(slot_id).cloned().unwrap_or_default();

    info!("Tokenized into {} tokens", tokens.len());

    // Process tokens through the handler system
    let result = system().process(ProcessMode::OneShot, Some(&content));

    info!(
        "Processing complete: {} tokens, {} patterns matched, {} handlers invoked",
        result.tokens_processed, result.patterns_matched, result.handlers_invoked
    );

    // Collect converted output from database reports
    let mut output_parts: Vec<String> = Vec::new();
    output_parts.push(format!("// Generated by c2r from: {}", input_file));
    output_parts.push(String::new());

    // Get conversion reports from the database
    for entry in system().by_category("conversion") {
        if let Some(message) = entry.get_string_attr("message") {
            // Extract the rust code from "Converted to: <code>"
            if let Some(code) = message.strip_prefix("Converted to: ") {
                output_parts.push(code.to_string());
            }
        }
    }

    // If no conversions were made, show what we have
    if output_parts.len() <= 2 {
        output_parts.push("// No conversions performed yet".to_string());
        output_parts.push(format!("// Tokens: {}", tokens.len()));

        // Show first few tokens for debugging
        if !tokens.is_empty() {
            output_parts.push("// First tokens:".to_string());
            for (i, token) in tokens.iter().take(10).enumerate() {
                output_parts.push(format!("//   {}: {}", i, token));
            }
            if tokens.len() > 10 {
                output_parts.push(format!("//   ... and {} more", tokens.len() - 10));
            }
        }
    }
    ...
    for error in &result.errors {
        output_parts.push(format!("// Error: {}", error));
    }

    let converted = output_parts.join("\n");
