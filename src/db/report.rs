...
impl Report {
    /// Create a new report
    pub fn new(id: impl Into<String>, report_type: impl Into<String>, message: impl Into<String>) -> Self {
        Self {
            id: id.into(),
    ...
    /// Add a relation
    pub fn with_relation(mut self, rel_type: impl Into<String>, entity_id: impl Into<String>) -> Self {
    ...
    pub fn to_entry(&self) -> Entry {
        ...
        if !self.fields.is_empty() {
            let fields_vec: Vec<Entry> = self.fields.iter()
                .map(|(k, v)| {
        ...
        if !self.relations.is_empty() {
            let rels_vec: Vec<Entry> = self.relations.iter()
                .map(|(k, v)| {
    ...
    pub fn from_entry(entry: &Entry) -> Option<Report> {
        ...
        let report_type = entry.get_string_attr("report_type").unwrap_or("").to_string();
...
impl Build for Report {
    ...
    fn category(&self) -> Option<&str> {
        if self.report_type.is_empty() { None } else { Some(&self.report_type) }
    }
}
...
fn format_table(report: &Report) -> String {
    ...
    writeln!(out, "| {:76} |", format!("{}: {}", report.report_type, report.message)).unwrap();
    writeln!(out, "+{}+", "-".repeat(78)).unwrap();
...
fn format_json(report: &Report) -> String {
    ...
    if !report.tags.is_empty() {
        writeln!(out, "  \"tags\": [{}]", report.tags.iter().map(|t| format!("\"{}\"", t)).collect::<Vec<_>>().join(", ")).unwrap();
    }
...
fn format_compact(report: &Report) -> String {
    format!("[{}] {} ({})", report.report_type, report.message, report.id)
}
...
fn format_tree(report: &Report, depth: usize) -> String {
    ...
    writeln!(out, "{}{}[{}] {}", prefix, branch, report.report_type, report.message).unwrap();
...
mod tests {
    ...
    fn test_report_formatting() {
        let report = Report::new("r1", "test", "Test message")
