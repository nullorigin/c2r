...
impl Display for Token {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Token::a(a, s) => write!(f, "{}", from_utf8(&a[..s.clone()]).unwrap_or("Invalid UTF-8")),
            Token::b(a) => write!(f,"{}", *a as char),
            Token::c(c) => write!(f, "{}", c.to_string()),
            Token::f(n) => write!(f, "{}", n.to_string()),
            Token::i(n) => write!(f, "{}", n.to_string()),
            Token::p(p, s) => unsafe { write!(f, "{}", str::from_utf8_unchecked(from_raw_parts(*p, *s))) },
...
impl Ord for Token {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        match (self, other) {
            ...
            (Token::a(a, s1), Token::c(b)) => if *s1 == 1 { (a[0] as char).cmp(b) } else { std::cmp::Ordering::Greater },
            (Token::a(a, s), Token::f(b)) => unsafe { std::str::from_utf8_unchecked(&a[..*s]).cmp(&b.to_string()) },
            (Token::a(a, s), Token::i(b)) => s.cmp(&b.to_ne_bytes().len()).then(a[..*s].cmp(&b.to_ne_bytes())),
            (Token::a(a, s), Token::p(b, s2)) => unsafe { s.cmp(s2).then(a[..*s].cmp(std::slice::from_raw_parts(*b, *s2))) },
            (Token::a(a, s1), Token::s(b)) => unsafe { std::str::from_utf8_unchecked(&a[..*s1]).cmp(b) },
            (Token::a(a, s1), Token::l(b)) => unsafe { std::str::from_utf8_unchecked(&a[..*s1]).cmp(b) },
            (Token::a(a, s), Token::u(b)) => s.cmp(&b.to_ne_bytes().len()).then(a[..*s].cmp(&b.to_ne_bytes())),
            ...
            (Token::b(a), Token::p(b, _s)) => unsafe { a.cmp(&b.read()) },
            ...
            // Token::ch matches
            (Token::c(a), Token::a(b, s)) => if *s == 1 { a.cmp(&(b[0] as char)) } else { std::cmp::Ordering::Greater },
            ...
            (Token::c(a), Token::p(b, s)) => unsafe { 1.cmp(s).then(a.cmp(&(b.read() as char))) },
            ...
            (Token::c(a), Token::v(b)) => if b.len() == 1 { a.cmp(&(b[0] as char)) } else { std::cmp::Ordering::Less },

            // Token::fl matches
            (Token::f(a), Token::a(b, s)) => unsafe { a.to_string().cmp(&String::from_utf8_unchecked(b[..*s].to_vec())) },
            ...
            (Token::f(a), Token::p(b, s)) => unsafe { a.to_string().cmp(&String::from_utf8_unchecked(from_raw_parts(*b, *s).to_vec())) },
            ...
            // Token::int matches
            (Token::i(a), Token::a(b, s)) => a.to_ne_bytes().len().cmp(s).then(a.to_ne_bytes().as_ref().cmp(&b[..*s])),
            ...
            (Token::i(a), Token::p(b, s)) => unsafe { a.to_ne_bytes().to_vec().cmp(&from_raw_parts(b.cast_const(), *s).to_vec()) },
            (Token::i(a), Token::s(b)) => a.to_string().cmp(&*b),
            (Token::i(a), Token::l(b)) => a.to_string().cmp(&b.to_string()),
            (Token::i(a), Token::u(b)) => (*a as u128).cmp(b),
            (Token::i(a), Token::v(b)) => unsafe { a.to_string().cmp(&String::from_utf8_unchecked(b.clone())) },

            // Token::ptr matches
            (Token::p(a, s1), Token::a(b, s2)) => unsafe { s1.cmp(s2).then(from_raw_parts(*a, *s1).cmp(&b[..*s2])) },
            (Token::p(a, s), Token::b(b)) => unsafe { s.cmp(&1).then(a.read().cmp(b)) },
            (Token::p(a, s), Token::c(b)) => unsafe { s.cmp(&1).then((a.read() as char).cmp(b)) },
            (Token::p(a, s), Token::f(b)) => unsafe { String::from_utf8_unchecked(from_raw_parts(*a, *s).to_vec()).cmp(&b.to_string()) },
            (Token::p(a, s), Token::i(b)) => unsafe { s.cmp(&b.to_ne_bytes().len()).then(from_raw_parts(*a, *s).cmp(&b.to_ne_bytes())) },
            (Token::p(a, s1), Token::p(b, s2)) => s1.cmp(s2).then(a.cmp(b)),
            (Token::p(a, s), Token::s(b)) => unsafe { from_raw_parts(*a, *s).cmp(b.as_bytes()) },
            (Token::p(a, s), Token::l(b)) => unsafe { from_raw_parts(*a, *s).cmp(b.as_bytes()) },
            (Token::p(a, s), Token::u(b)) => unsafe { s.cmp(&b.to_ne_bytes().len()).then(from_raw_parts(*a, *s).cmp(&b.to_ne_bytes())) },
            (Token::p(a, s), Token::v(b)) => unsafe { s.cmp(&b.len()).then(from_raw_parts(*a, *s).cmp(b)) },

            // Token::s matches
            (Token::s(a), Token::a(b, s)) => unsafe { a.cmp(&String::from_utf8_unchecked(b[..*s].to_vec())) },
            ...
            (Token::s(a), Token::p(b, s)) => unsafe { a.as_bytes().cmp(from_raw_parts(*b, *s)) },
            ...
            (Token::l(a), Token::p(b, s)) => unsafe { a.as_bytes().cmp(from_raw_parts(*b, *s)) },
            ...
            // Token::uint matches
            (Token::u(a), Token::a(b, s)) => a.to_ne_bytes().len().cmp(s).then(a.to_ne_bytes().as_ref().cmp(&b[..*s])),
            ...
            (Token::u(a), Token::p(b, s)) => unsafe { a.to_ne_bytes().to_vec().cmp(&from_raw_parts(b.cast_const(), *s).to_vec()) },
            (Token::u(a), Token::s(b)) => a.to_string().cmp(b),
            (Token::u(a), Token::l(b)) => a.to_string().cmp(&b.to_string()),
            (Token::u(a), Token::u(b)) => a.cmp(b),
            (Token::u(a), Token::v(b)) => a.to_string().cmp(&String::from_utf8(b.clone()).unwrap_or("Invalid UTF-8".to_string())),

            // Token::vec matches
            (Token::v(a), Token::a(b, s)) => a.cmp(&b[..*s].to_vec()),
            (Token::v(a), Token::b(b)) => a.len().cmp(&1).then(a[0].cmp(b)),
            (Token::v(a), Token::c(b)) => if a.len() == 1 { (a[0] as char).cmp(b) } else { std::cmp::Ordering::Greater },
            (Token::v(a), Token::f(b)) => String::from_utf8(a.clone()).unwrap_or("Invalid UTF-8".to_string()).cmp(&b.to_string()),
            (Token::v(a), Token::i(b)) => a.len().cmp(&b.to_ne_bytes().len()).then(a.cmp(&b.to_ne_bytes().to_vec())),
            (Token::v(a), Token::p(b, s)) => unsafe { a.cmp(&from_raw_parts(*b, *s).to_vec()) },
            (Token::v(a), Token::s(b)) => String::from_utf8(a.clone()).unwrap_or("Invalid UTF-8".to_string()).cmp(b),
            (Token::v(a), Token::l(b)) => a.cmp(&b.as_bytes().to_vec()),
            (Token::v(a), Token::u(b)) => a.len().cmp(&b.to_ne_bytes().len()).then(a.cmp(&b.to_ne_bytes().to_vec())),
...
impl Token {
    ...
    pub fn is_numeric(&self) -> bool {
        match self {
            ...
            Token::b(_) => true,
            Token::c(_) => true,
            Token::s(s) => s.parse::<i128>().is_ok() || s.parse::<u128>().is_ok() || s.parse::<f64>().is_ok(),
            Token::l(s) => s.parse::<i128>().is_ok() || s.parse::<u128>().is_ok() || s.parse::<f64>().is_ok(),
            _ => false,
        }
    }
    ...
    pub fn stok(&self) -> Token {
        match self.clone() {
            ...
            Token::p(p, s) => unsafe { Token::s(String::from_raw_parts(p as *mut _, s, s)) },
            Token::s(s) => Token::s(s.clone()),
            Token::l(s) => Token::s(s.to_string()),
            Token::u(u) => Token::s(u.to_string()),
            Token::v(v) => Tokenizer::from_utf8(&v),
        }
    }
    ...
    pub fn op(self, other:Token, op:&str) -> Option<Token> {
        match (self,other,op) {
            ...
            (Token::u(a), Token::u(b), op) => match op {
                ...
                "/" => if b != 0 { Some(Token::u(a / b)) } else { None },
                "%" => if b != 0 { Some(Token::u(a % b)) } else { None },
                "^" => Some(Token::u(a ^ b)),
                "&" => Some(Token::u(a & b)),
                "|" => Some(Token::u(a | b)),
            ...
            (Token::b(a), Token::b(b), op) => match op {
                ...
                "/" => if b != 0 { Some(Token::b(a / b)) } else { None },
                "%" => if b != 0 { Some(Token::b(a % b)) } else { None },
                "^" => Some(Token::b(a ^ b)),
                "&" => Some(Token::b(a & b)),
                "|" => Some(Token::b(a | b)),
    ...
    pub fn tokens_to_string(tokens:&[Token],start_idx:usize,end_idx:usize) -> Option<String> {
        ...
        Some(tokens[start_idx..end_idx].iter().map(|t| t.to_string()).collect::<String>())
    }
...
impl PartialEq for Token {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Token::a(a, s1), Token::p(b, s2)) => unsafe { *s1 == *s2 && &a[..*s1] == std::slice::from_raw_parts(*b, *s2) },
            ...
            (Token::b(a), Token::p(b, _s)) => unsafe { *a == b.read() },
            ...
            (Token::c(a), Token::p(b, s)) => unsafe { *s == 1 && *a == (b.read() as char) },
            ...
            (Token::f(a), Token::p(b, s)) => unsafe { a.to_string().as_bytes() == std::slice::from_raw_parts(*b, *s) },
            ...
            (Token::i(a), Token::p(b, s)) => unsafe { a.to_string().as_bytes() == std::slice::from_raw_parts(*b, *s) },
            ...
            (Token::u(a), Token::p(b, s)) => unsafe { a.to_string().as_bytes() == std::slice::from_raw_parts(*b, *s) },
            (Token::u(a), Token::s(b)) => a.to_string() == *b,
            (Token::u(a), Token::l(b)) => a.to_string() == *b,
            (Token::u(a), Token::v(b)) => a.to_string().as_bytes() == b,

            // Token::ptr matches
            (Token::p(a, s), Token::a(b, s2)) => unsafe { *s == *s2 && std::slice::from_raw_parts(*a, *s) == &b[..*s2] },
            (Token::p(a, s), Token::b(b)) => unsafe { *s == 1 && a.read() == *b },
            (Token::p(a, s), Token::c(b)) => unsafe { *s == 1 && (a.read() as char) == *b },
            (Token::p(a, s), Token::f(b)) => unsafe { std::slice::from_raw_parts(*a, *s) == b.to_string().as_bytes() },
            (Token::p(a, s), Token::i(b)) => unsafe { std::slice::from_raw_parts(*a, *s) == b.to_string().as_bytes() },
            (Token::p(a, s), Token::u(b)) => unsafe { std::slice::from_raw_parts(*a, *s) == b.to_string().as_bytes() },
            (Token::p(a, s1), Token::p(b, s2)) => s1 == s2 && a == b,
            (Token::p(a, s), Token::s(b)) => unsafe { *s == b.len() && std::slice::from_raw_parts(*a, *s) == b.as_bytes() },
            (Token::p(a, s), Token::l(b)) => unsafe { *s == b.len() && std::slice::from_raw_parts(*a, *s) == b.as_bytes() },
            (Token::p(a, s), Token::v(b)) => unsafe { *s == b.len() && std::slice::from_raw_parts(*a, *s) == &b[..] },
            ...
            (Token::s(a), Token::p(b, s)) => unsafe { a.as_bytes() == std::slice::from_raw_parts(*b, *s) },
            ...
            (Token::l(a), Token::p(b, s)) => unsafe { a.as_bytes() == std::slice::from_raw_parts(*b, *s) },
            ...
            (Token::v(a), Token::p(b, s)) => unsafe { a == std::slice::from_raw_parts(*b, *s) },
...
impl Tokenizer {
    ...
    pub fn tokenize(mut self,content:Vec<u8>, context: &mut Context) -> Result<Vec<Token>, ConversionError> {
        ...
        if !self.content.is_empty() {
            ...
            debug!("First 100 chars: {:?}", from_utf8(preview).unwrap_or("Invalid UTF-8"));
        }
        ...
        while let Some(token) = self.next_token() {
            ...
            if token_count % 10 == 0 || token_count == 1 || token_count == tokens.len() {
                ...
                match &token {
                    ...
                    _ => {debug!("Found token {} at token #{} (cursor {})",token.stok(),token_count, self.cursor);}
                }
        ...
        if self.cursor < self.content.len() {
            ...
            if let Ok(content) = String::from_utf8(self.content.clone()) {
                let (processed, remaining) = content.split_at(self.cursor);
                debug!("Processed content:\n{}", processed);
                warn!("Remaining content:\n{}", remaining);
            } else {
                return Err(ConversionError::general("Invalid UTF-8"));
            }
        }
    ...
    pub fn next_token(&mut self) -> Option<Token> {
        ...
        if b.is_ascii_alphabetic() || b == b'_' {
        ...
        } else if b.is_ascii_digit() || (b == b'.' && self.peek().map_or(false, |next| next.is_ascii_digit())) {
            ...
            while self.cursor < self.content.len() {
                ...
                if current.is_ascii_digit() {
                    ...
                    _has_decimal = true;
                    self.cursor += 1;
                } else if (current == b'e' || current == b'E') && self.cursor + 1 < self.content.len() {
                    ...
                    if next.is_ascii_digit() || next == b'+' || next == b'-' {
                        ...
                        while self.cursor < self.content.len() && self.cursor_byte().is_ascii_digit() {
                            self.cursor += 1;
                        }
                    }
                    break;
    ...
    fn skip_line_comment(&mut self) {
        ...
        while self.cursor < self.content.len() && self.content[self.cursor] != b'\n' {
            ...
            if self.content[self.cursor] == b'\\' && self.cursor + 1 < self.content.len() && self.content[self.cursor + 1] == b'\n' {
    ...
    fn skip_block_comment(&mut self) {
        ...
        while self.cursor + 1 < self.content.len() {
            ...
            if chars_consumed > max_comment_length {
                ...
                error!(
                    "Comment starts with: {}",
                    String::from_utf8_lossy(&self.content[start_pos..std::cmp::min(start_pos + 40, self.content.len())])
                );
    ...
    fn recover_from_unclosed_comment(&mut self, comment_start_pos: usize) {
        ...
        let type_patterns = [
            ...
            "struct ", "enum ", "union ",
            // Combinations
            "static int ", "static void ", "extern int ", "extern void ",
            ...
            while let Some(idx) = content[search_pos..].find(pattern) {
                ...
                if line.contains('(') && line.contains(')') {
                    ...
                    if let Some(paren_pos) = line.find('(') {
                        ...
                        if !before_paren.is_empty() && before_paren.chars().all(|c| c.is_alphanumeric() || c == '_') {
                            candidates.push((pos, line.to_string()));
                            debug!("Found potential function at position {}: {}", pos, line.trim());
                        }
                    }
                }
        ...
        if !candidates.is_empty() {
            ...
            info!("Resuming tokenization at function declaration at position {}: {}", resume_pos, decl.trim());
            self.cursor = *resume_pos;
            warn!("/* Tokenizer recovered from unclosed comment. Resuming at function declaration */");
            return;
        }
        ...
        while pos + 1 < content.len() {
            if content.as_bytes()[pos] == b'}' {
                ...
                while after_brace < content.len() && content.as_bytes()[after_brace].is_ascii_whitespace() {
                    after_brace += 1;
                }
                ...
                if after_brace < content.len() {
                    ...
                    if type_patterns.iter().any(|&pattern| next_line.starts_with(pattern)) &&
                        next_line.contains('(') {
                        debug!("Found potential function after brace at position {}: {}", after_brace, next_line.trim());
                        self.cursor = after_brace;
                        warn!("/* Tokenizer recovered from unclosed comment after closing brace */");
                        return;
                    }
                }
            }
        ...
        debug!("No recovery points found. Continuing from position {}", original_pos);
    ...
    pub fn process_tokens(&mut self, tokens: &[Token], context: &mut Context) -> Result<ProcessedResult, ConversionError> {
        ...
        for mut handler in context.handlers.get_handlers() {
            handler_stats.insert(handler.name().to_string(), 0);
        }
        let mut processed = &mut handlers.process(tokens,context).unwrap();
        ...
        let mut result = String::with_capacity(tokens.len() * 2); // Pre-allocate memory
            ...
            if context.get_verbosity() >= 2 {
                match &processed.result {
                    HandlerResult::Converted(_, _, id) | HandlerResult::Replaced(_, _, id) => {
            ...
            match &processed.result {
                HandlerResult::Processed(_, code, _) => {
                ...
                HandlerResult::Replaced(_, code, _) => {
                    result.push_str(code);
                    if !code.ends_with('\n') {
                        result.push('\n');
                    }
                },
                HandlerResult::NotHandled(Some(tokens), _) => {
                    unhandled_count += 1;
                    // For unhandled tokens, generate a comment with the original _code
                    let unhandled_tokens = &tokens[processed.start_idx..processed.start_idx + processed.tokens_consumed];
                    ...
                    if !unhandled_token_str.trim().is_empty() &&
                        !unhandled_tokens.iter().all(|t| matches!(t, tok!(' ') | tok!('\t') | tok!('\n'))) {
                ...
                HandlerResult::Redirected(_tokens, _code, id1, id2) => {
                    error_count += 1;
                ...
                HandlerResult::Extracted(_, _, _) => {},
                HandlerResult::NotHandled(None, _) => {}
            }
        ...
        if context.get_verbosity() >= 1 {
            ...
            footer.push_str(&format!("// Processed tokens: {} ({:.1}%)\n", processed_tokens, processed_pct));
            ...
            if !handler_stats.is_empty() && context.get_verbosity() >= 2 {
                ...
                for (handler, count) in sorted_stats {
                    if *count > 0 {
                        ...
                        footer.push_str(&format!("//   {}: {} tokens ({:.1}%)\n", handler, count, handler_pct));
                    }
                }
            }
