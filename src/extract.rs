...
impl fmt::Display for ExtractedArray {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.is_declaration {
            write!(f, "let mut {}: [{}; {}] = [Default::default(); {}];", 
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.expression_type.as_str() {
            "ternary" => {
                ...
                if parts.len() == 2 {
                    write!(f, "if {} {{ {} }} else {{ {} }}", self.left_operand, parts[0], parts[1])
                } else {
                    write!(f, "{} {} {}", self.left_operand, self.operator, self.right_operand)
                }
            },
            _ => write!(f, "{} {} {}", self.left_operand, self.operator, self.right_operand)
        }
    }
}
...
impl fmt::Display for ExtractedControlFlow {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let condition_str = self.condition.iter()
            .map(|t| t.to_string())
            .collect::<Vec<_>>()
            .join(" ");
        ...
        match self.control_type.as_str() {
            ...
            _ => write!(f, "{} {} {{ {} }}", self.control_type, condition_str, body_str)
        }
    }
}
...
fn token_to_rust(token: &Token) -> String {
    match token {
        ...
        tok!(',') => ",".to_string(),
        tok!('*') => "*".to_string(),
        tok!(id) if *id == "==" || *id == "!=" || *id == "<=" || *id == ">=" || *id == "&&" || *id == "||" => id.to_string(),
        tok!(id) if *id == "++" => {
...
pub fn is_c_qualifier_or_specifier(token: &Token) -> bool {
    !token.matches(vec![
        tok!("const"), tok!("volatile"), tok!("static"), 
        tok!("extern"), tok!("register"), tok!("inline")
    ]).iter().all(|x| x.is_none())
}

/// Check if a token is a C keyword that can be used in control flow
pub fn is_c_control_keyword(token: &Token) -> bool {
    !token.matches(vec![
        tok!("if"), tok!("else"), tok!("while"), tok!("for"), 
        tok!("do"), tok!("switch"), tok!("case"), tok!("default"),
        tok!("return"), tok!("break"), tok!("continue"), tok!("goto")
    ]).iter().all(|x| x.is_none())
}
...
pub fn to_rust_string(tokens: &[Token]) -> Option<String> {
    ...
    Token::tokens_to_string(convert::convert_type_tokens(tokens.to_vec()).as_slice(),0,tokens.len())
}
