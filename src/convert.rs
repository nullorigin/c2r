use crate::{matches, tok, Id, Token};
...
impl fmt::Display for ConvertedControlFlow {
    ...
    pub fn convert_type_token(c_token: &Token) -> Option<String> {
        ...
        MAP.get(type_str.trim()).map(|rust_type| rust_type.to_string())
    }
    ...
    pub fn convert_type_tokens(c_type: Vec<Token>) -> Option<Token> {
        ...
        for (t) in c_type.iter() {
            let s = t.to_string();
             if s.trim() != "**" && s.trim() != "&&" && s.trim() != "*" && s.trim() != "&" && s.trim() != "" {
        ...
        MAP.get(&c_type_str).map(|rust_type| Token::s(rust_type.to_string()))
    }
    pub fn is_c_keyword(token: Token) -> bool {
        ["auto".to_string(), "break".to_string(), "case".to_string(), "char".to_string(), "const".to_string(), "continue".to_string(),"default".to_string(),"do".to_string(),"else".to_string(),"enum".to_string(),"extern".to_string(),"float".to_string(),"for".to_string(),"goto".to_string(),"if".to_string(),"inline".to_string(),"int".to_string(),"long".to_string(),"register".to_string(),"return".to_string(), "signed".to_string(),"sizeof".to_string(),"static".to_string(),"struct".to_string(), "switch".to_string(),"typedef".to_string(),"union".to_string(),"unsigned".to_string(),"volatile".to_string(),"while".to_string()].contains(&token.to_string())
    }
    pub fn create_map() -> HashMap<String,String> {
        ...
        for (c_type,rust_type) in hm1.iter_mut() {
            ...
            hm2.insert(format!("const {}*", c_type), format!("*const {}", rust_type));

            // Generate double pointer variants
            hm2.insert(format!("{}**", c_type), format!("*mut *mut {}", rust_type));
            hm2.insert(format!("const {}**", c_type), format!("*const *const {}", rust_type));
