...
use crate::{Context, Entry, HandlerPhase, PatternResult, Registry, ReportLevel, Result, Token, context, entry}; // Adaptivizer functionality moved to handler::Handlizer
...
impl Samplizer {
    ...
    pub fn generate_adaptive_variations(
        context: &mut Context,
        base_pattern: &crate::pattern::HandlerPattern,
        deviation_bounds: f64,
    ) -> crate::Result<Vec<SamplizerPattern>> {
        ...
        for priority_delta in [-2, -1, 1, 2] {
            ...
            if deviation <= deviation_bounds {
                let pattern_id = Id::gen_name("adaptive_priority_var");
                let variation = SamplizerPattern {
                    pattern_id: pattern_id.clone(),
                    pattern_name: format!("{} (Priority {})", base_pattern.description, new_priority),
        ...
        for (i, new_min) in min_variations.iter().enumerate() {
            ...
            if deviation <= deviation_bounds {
                let pattern_id = Id::gen_name("adaptive_tokens_var");
                let variation = SamplizerPattern {
                    pattern_id: pattern_id.clone(),
                    pattern_name: format!("{} (Min tokens: {})", base_pattern.description, new_min),
                    pattern_type: SegmentType::Function,
                    confidence_score: 1.0 - deviation,
                    extraction_pattern: Some(format!("Token count variation of {}", base_pattern.id)),
                    c_tokens: Vec::new(),
                    rust_tokens: Vec::new(),
                };
                variations.push(variation);
    ...
    pub fn learn_from_adaptive_success(
        ...
        let pattern_id = Id::gen_name("adaptive_learned");
        ...
        self.generated_patterns.insert(pattern_id.clone(), learned_pattern);

        // Update token analysis data with successful pattern
        let token_sequence = tokens.iter().map(|t| t.to_string()).collect::<Vec<_>>().join(" ");
    ...
    pub fn generate_fractal_explorations(
        context: &mut Context,
        center_pattern: &crate::pattern::HandlerPattern,
        ...
        for branch in 0..num_radial_branches {
            ...
            for distance_level in 1..=depth {
                ...
                let pattern_id = Id::gen_name("fractal_exploration");
                let variation = SamplizerPattern {
                    pattern_id: pattern_id.clone(),
                    pattern_name: format!(
                        "{} (Fractal branch {}, level {})",
                        center_pattern.description, branch, distance_level
                    ),
                    pattern_type: SegmentType::Function,
                    confidence_score: (1.0 - distance).max(0.1),
                    extraction_pattern: Some(format!("Fractal exploration from {}", center_pattern.id)),
                    c_tokens: Vec::new(),
                    rust_tokens: Vec::new(),
                };
    ...
    pub fn analyze_file_pair<P: AsRef<Path>>(
        ...
        let aligned_pair = match Self::align_segments(context, file_pair) {
            Ok(pair) => pair,
            Err(e) => {
                report!(
                    context,
        ...
        let mut patterns = Vec::with_capacity(effective_max_patterns.min(aligned_pair.alignment_map.len()));
        ...
        if patterns.is_empty() && !aligned_pair.c_tokens.is_empty() && !aligned_pair.rust_tokens.is_empty() {
            ...
            for i in 0..fallback_count {
                ...
                let pattern_id = format!("fallback_{}_{}", base_name, i);
                ...
                patterns.push(SamplizerPattern {
                    ...
                    c_tokens: aligned_pair.c_tokens.iter().take(3).map(|t| t.to_string()).collect(),
                    rust_tokens: aligned_pair.rust_tokens.iter().take(3).map(|t| t.to_string()).collect(),
                });
            }
    ...
    fn parse_file_pair<P: AsRef<Path>>(
        ...
        if !rust_file_path.exists() {
            return Err(C2RError::new(
                ...
                Some(format!("Rust file does not exist: {}", rust_file_path.display())),
            ));
        }
        ...
        let c_content = fs::read_to_string(c_file_path).map_err(|e| {
            C2RError::new(
                ...
                Some(format!("Failed to read C file {}: {}", c_file_path.display(), e)),
            )
        })?;
        ...
        let rust_content = fs::read_to_string(rust_file_path).map_err(|e| {
            C2RError::new(
                ...
                Some(format!("Failed to read Rust file {}: {}", rust_file_path.display(), e)),
            )
        })?;

        // Validate content is not empty
        if c_content.is_empty() && rust_content.is_empty() {
            return Ok(FilePair {
        ...
        let c_tokens = if c_content.is_empty() {
            Vec::new()
        } else {
            context.tokenizer.tokenize(c_content.into_bytes()).map_err(|e| {
                C2RError::new(
                    Kind::Io,
                    Reason::Failed("to tokenize C file content"),
                    Some(format!("C tokenization failed for {}: {}", c_file_path.display(), e)),
                )
            })?
        };

        let rust_tokens = if rust_content.is_empty() {
            Vec::new()
        } else {
            context.tokenizer.tokenize(rust_content.into_bytes()).map_err(|e| {
                C2RError::new(
                    Kind::Io,
                    Reason::Failed("to tokenize Rust file content"),
                    Some(format!("Rust tokenization failed for {}: {}", rust_file_path.display(), e)),
                )
            })?
        };
    ...
    fn align_segments(context: &mut Context, mut file_pair: FilePair) -> Result<FilePair> {
        ...
        let mut alignment_results: Vec<Vec<SegmentAlignment>> = Vec::with_capacity(window_sizes.len());
        ...
        let mut alignments: Vec<SegmentAlignment> = alignment_results
            .into_iter()
            .flatten()
            .collect();
        ...
        alignments.sort_by(|a, b| {
            ...
            match b.confidence.partial_cmp(&a.confidence).unwrap_or(std::cmp::Ordering::Equal) {
        ...
        alignments.dedup_by(|a, b| {
            ...
            let size_similar = (a.c_segment.1 - a.c_segment.0).abs_diff(b.c_segment.1 - b.c_segment.0) <= 1;
    ...
    fn find_high_quality_alignments(
        ...
        for &window_size in &window_sizes {
            ...
            let result = Self::process_window_size(c_tokens, rust_tokens, window_size, quality_threshold);
            if !result.is_empty() {
                alignment_results.push(result);
            }
        }
        ...
        let high_quality_count = alignments.iter().take(1500).filter(|a| a.confidence > 0.85).count();
    ...
    fn parse_file_to_tokens<P: AsRef<Path>>(context: &mut Context, file_path: P) -> Result<Vec<Token>> {
        ...
        let content = fs::read_to_string(file_path).map_err(|e| {
            C2RError::new(
                ...
                Some(format!("Failed to read file {}: {}", file_path.display(), e)),
            )
        })?;
        ...
        let tokens = context.tokenizer.tokenize(content.into_bytes()).map_err(|e| {
            C2RError::new(
                Kind::Io,
                Reason::Failed("to tokenize file content"),
                Some(format!("Tokenization failed for {}: {}", file_path.display(), e)),
            )
        })?;
    ...
    fn is_window_meaningful(tokens: &[Token]) -> bool {
        ...
        let meaningful_keywords = token_strings
            ...
            .filter(|t| {
                matches!(
                    t.as_str(),
                    "struct" | "enum" | "typedef" | "int" | "char" | "float" | "void" |
                    "if" | "while" | "for" | "return" | "static" | "const" | "unsigned" |
                    "signed" | "long" | "short" | "double" | "extern" | "inline"
                )
            })
            .count();
        ...
        let identifiers = token_strings
            ...
            .filter(|t| {
                ...
                !s.is_empty() 
                    ...
                    && !matches!(s, "struct" | "enum" | "typedef" | "int" | "char" | "float" | "void" |
                        "if" | "while" | "for" | "return" | "static" | "const" | "unsigned" |
                        "signed" | "long" | "short" | "double" | "extern" | "inline")
            })
            .count();
    ...
    fn calculate_similarity(c_tokens: &[Token], rust_tokens: &[Token]) -> f64 {
        ...
        let common_count = c_strings
            ...
            .count();
        let direct_similarity = common_count as f64 / c_strings.len().max(rust_strings.len()) as f64;
    ...
    fn calculate_pattern_complexity(c_strings: &[String], rust_strings: &[String]) -> f64 {
    ...
    fn is_type_keyword(token_string: &str) -> bool {
        matches!(
            ...
            | "uint64_t" | "intmax_t" | "uintmax_t" | "int_fast8_t" 
    ...
    pub fn validate_patterns(&mut self, context: &mut Context, test_cases: &[FilePair]) -> Result<()> {
        report!(
            context,
            "samplizer",
            "validate_patterns",
    ...
    fn test_pattern_on_file_pair(
        ...
        for alignment in &file_pair.alignment_map {
            ...
            if pattern.pattern_type == alignment.segment_type {
                ...
                let c_segment = &file_pair.c_tokens[alignment.c_segment.0..alignment.c_segment.1];
                let rust_segment = &file_pair.rust_tokens[alignment.rust_segment.0..alignment.rust_segment.1];
                
                // Convert tokens to strings for comparison
                let c_tokens_str: Vec<String> = c_segment.iter().map(|t| t.to_string()).collect();
                let rust_tokens_str: Vec<String> = rust_segment.iter().map(|t| t.to_string()).collect();
    ...
    fn tokens_are_similar(token1: &str, token2: &str) -> bool {
        ...
        (is_identifier(token1) && is_identifier(token2) && token1.len() > 2 && token2.len() > 2)
    }
    ...
    fn integrate_pattern_into_patternizer(
        ...
        if let Some(extraction_pattern) = &pattern.extraction_pattern {
            let handler_pattern = Samplizer::convert_to_handler_pattern(pattern, extraction_pattern)?;
            ...
            context.patternizer.register_pattern(handler_type.to_string(), handler_pattern);
    ...
    /// Pattern matching implementation for token sequences
    fn match_pattern(&self, pattern_name: &str, tokens: &[Token]) -> PatternResult {
        ...
        match pattern_name {
            ...
            "enum_declaration" => {
                self.match_enum_pattern(&token_strings)
            }
            "variable_declaration" => {
                self.match_variable_pattern(&token_strings)
            }
            "union_declaration" => {
                self.match_union_pattern(&token_strings)
            }
            "typedef_declaration" => {
                self.match_typedef_pattern(&token_strings)
            }
            "macro_definition" => {
                self.match_macro_pattern(&token_strings)
            }
    ...
    fn match_function_pattern(&self, tokens: &[String]) -> PatternResult {
        ...
        while i < tokens.len() && matches!(tokens[i].as_str(), "static" | "extern" | "inline" | "const") {
            consumed += 1;
            i += 1;
        }
    ...
    fn match_variable_pattern(&self, tokens: &[String]) -> PatternResult {
        ...
        // Look for optional storage class specifiers
        while i < tokens.len() && matches!(tokens[i].as_str(), "static" | "extern" | "const" | "volatile") {
            consumed += 1;
            i += 1;
        }
    ...
    fn match_typedef_pattern(&self, tokens: &[String]) -> PatternResult {
        ...
        if i < tokens.len() && matches!(tokens[i].as_str(), "struct" | "union" | "enum") {
            ...
            matches!(tokens[i].as_str(), "int" | "char" | "float" | "double" | "void" | "long" | "short")
        ) {
            consumed += 1;
            i += 1;
        }
    ...
    pub fn analyze_with_range(
        ...
        let range_tokens = if token_range.end <= tokens.len() {
        ...
        for pattern_name in patterns {
            // Analyze pattern confidence using samplizer's pattern analysis
            let pattern_confidence = self
                .analyze_pattern_confidence(pattern_name, range_tokens)
                .unwrap_or(0.3); // Fallback confidence if analysis fails
        ...
        let confidence = if pattern_count > 0 {
            ...
            0.2 // Low default confidence when no patterns
        };
    ...
    fn extract_captures_from_tokens(
        ...
        match pattern_name {
            "function_declaration" | "function_definition" => {
                self.extract_function_captures(tokens, captures, consumed_tokens)?;
            }
            "struct_declaration" | "struct_definition" => {
                self.extract_struct_captures(tokens, captures, consumed_tokens)?;
            }
            "enum_declaration" => {
                self.extract_enum_captures(tokens, captures, consumed_tokens)?;
            }
            "union_declaration" => {
                self.extract_union_captures(tokens, captures, consumed_tokens)?;
            }
            "variable_declaration" => {
                self.extract_variable_captures(tokens, captures, consumed_tokens)?;
            }
            "typedef_declaration" => {
                self.extract_typedef_captures(tokens, captures, consumed_tokens)?;
            }
            "macro_definition" => {
                self.extract_macro_captures(tokens, captures, consumed_tokens)?;
            }
            _ => {
                // Enhanced generic extraction for unhandled patterns
                if !token_strings.is_empty() {
                    // Capture the pattern name and basic token information
                    captures.capture_text("pattern_type", pattern_name.to_string(), 0.7);
                    captures.capture_text("raw_tokens", token_strings.join(" "), 0.6);
                    
                    // Capture token range for debugging
                    captures.capture_range("token_range", 0..consumed_tokens, 0.8);

                    // Try to identify common elements with better heuristics
                    if let Some(first_token) = token_strings.first() {
                        if crate::handlers::common::is_identifier(first_token) {
                            captures.capture_text("identifier", first_token.clone(), 0.8);
                        }
                    }

                    // Look for common patterns in unknown structures
    ...
    fn extract_union_captures(
        ...
        let token_strings: Vec<String> = tokens.iter().take(consumed_tokens).map(|t| t.to_string()).collect();
        ...
        if let Some(union_pos) = token_strings.iter().position(|t| t == "union") {
            if union_pos + 1 < token_strings.len() {
                ...
                if crate::handlers::common::is_identifier(union_name) {
            ...
            if typedef_pos + 1 < token_strings.len() && token_strings[typedef_pos + 1] == "union" {
                ...
                if let Some(semicolon_pos) = token_strings.iter().rposition(|t| t == ";") {
                    if semicolon_pos > 0 {
                        ...
                        if crate::handlers::common::is_identifier(alias_name) {
    ...
    fn extract_variable_captures(
        ...
        for (i, token) in token_strings.iter().enumerate() {
            if matches!(token.as_str(), "int" | "char" | "float" | "double" | "void" | "long" | "short" | "unsigned" | "signed") {
                type_tokens.push(token.clone());
                found_type = true;
            } else if found_type && crate::handlers::common::is_identifier(token) && var_name.is_empty() {
    ...
    fn extract_typedef_captures(
        ...
        if let Some(typedef_pos) = token_strings.iter().position(|t| t == "typedef") {
            ...
            // Extract base type
            let type_tokens: Vec<String> = token_strings[typedef_pos + 1..token_strings.len().saturating_sub(2)]
                .iter()
                .cloned()
                .collect();
        ...
        if let Some(define_pos) = token_strings.iter().position(|t| t == "#define") {
            if define_pos + 1 < token_strings.len() {
                ...
                if define_pos + 2 < token_strings.len() && token_strings[define_pos + 2] == "(" {
                    ...
                    if let Some(close_paren) = token_strings.iter().position(|t| t == ")") {
                        ...
                        if !param_tokens.is_empty() {
                            captures.capture_text("macro_parameters", param_tokens.join(", "), 0.85);
                        }
                    }
                } else {
                ...
                let body_start = if token_strings.contains(&"(".to_string()) {
                    token_strings.iter().position(|t| t == ")").map(|p| p + 1).unwrap_or(define_pos + 2)
                } else {
                    define_pos + 2
                };
    ...
    fn extract_generic_patterns(&self, token_strings: &[String], captures: &mut CaptureSystem) -> Result<()> {
        ...
        if let Some(paren_pos) = token_strings.iter().position(|t| t == "(") {
            if paren_pos > 0 {
                ...
                if crate::handlers::common::is_identifier(func_name) {
        ...
        for (i, token) in token_strings.iter().enumerate() {
            if matches!(token.as_str(), "int" | "char" | "float" | "double" | "void" | "long" | "short" | "unsigned" | "signed") {
                ...
                if i + 1 < token_strings.len() && crate::handlers::common::is_identifier(&token_strings[i + 1]) {
    ...
    fn extract_function_captures(
        ...
        for i in 0..token_strings.len().saturating_sub(1) {
            if token_strings[i + 1] == "(" {
                ...
                if crate::handlers::common::is_identifier(potential_name) {
                    ...
                    if i > 0 {
                        let return_type_tokens: Vec<String> = token_strings[0..i]
                            .iter()
                            .filter(|&t| !matches!(t.as_str(), "static" | "extern" | "inline" | "const" | "volatile"))
                            .cloned()
                            .collect();
    ...
    fn extract_function_parameters(&self, param_tokens: &[String], captures: &mut CaptureSystem) -> Result<()> {
    ...
    fn extract_function_attributes(&self, token_strings: &[String], captures: &mut CaptureSystem) -> Result<()> {
    ...
    fn estimate_function_complexity(&self, token_strings: &[String]) -> String {
        ...
        let control_flow_count = token_strings.iter()
    ...
    fn extract_struct_captures(
        ...
        if let Some(struct_pos) = token_strings.iter().position(|t| t == "struct") {
            if struct_pos + 1 < token_strings.len() {
                ...
                if crate::handlers::common::is_identifier(struct_name) {
            ...
            if typedef_pos + 1 < token_strings.len() && token_strings[typedef_pos + 1] == "struct" {
                ...
                if let Some(semicolon_pos) = token_strings.iter().rposition(|t| t == ";") {
                    if semicolon_pos > 0 {
                        ...
                        if crate::handlers::common::is_identifier(alias) {
    ...
    fn extract_enum_captures(
        ...
        if let Some(enum_pos) = token_strings.iter().position(|t| t == "enum") {
            if enum_pos + 1 < token_strings.len() {
                ...
                if crate::handlers::common::is_identifier(enum_name) {
    ...
    fn load_learning_data(&mut self, context: &mut Context) -> Result<()> {
        ...
        let entry = context.registry.get_entry(learning_data_id).clone();
        match entry {
            Entry::Str(json_string) => {
                match crate::json::parsing::parse(&json_string) {
                    Ok(json_value) => {
    ...
    fn save_learning_data(&self, context: &mut Context) -> Result<()> {
        ...
        let json_string = json_data.dump();
        context.registry.insert(learning_data_id, crate::entry::Entry::Str(json_string));
        ...
        if let Err(e) = context.save_pattern_cache(&cache_path) {
            report!(
                context,
                "samplizer",
                "save_learning_data",
        ...
        // Access the match cache to learn from existing positive/negative results
            ...
            for (cache_key, pattern_result) in context.patternizer.match_cache.clone() {
                ...
                match pattern_result {
                    ...
                    | crate::pattern::PatternResult::Reject { .. } => {
...
impl CaptureSystem {
    ...
    pub fn capture_text(&mut self, key: &str, value: String, confidence: f64) {
        self.captures.insert(key.to_string(), CapturedValue::Text(value));
